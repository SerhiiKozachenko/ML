...
class NYTCovid:
    ...
    def updateCounty(self,
        ...
        url = url
        # same logic as for updateState but on county data
        s=requests.get(url).content
    ...
    def process(self):
        ...
        if self._stateupdated:
            ...
            for s in self.statelist:
                ...
                # Pandas data frame diff https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.diff.html
                ...
                # save it as new column where each row contains how many new cases compared to prev day (thisday.cases - prevday.cases)
        ...
        if self._countyupdated:
            # do the same on county data set
