...
from SECRETS import INST_PASSWORD, INST_USERNAME

username = os.environ.get('USERNAME') or INST_USERNAME
password = os.environ.get('PASSWORD') or INST_PASSWORD
...
def print_n_items_in_a_row(items, n:int):
    for i in range(0, len(items), n):
        print(*items[i:i+n], sep=', ')
...
def main():
    ...
    print("\n ---------- \n")
    print('you are following {} user(s):'.format(len(following_list)))
    print("\n ---------- \n")
    print_n_items_in_a_row(list(map(lambda u: u['username'], following_list)), 10)
    # for user in following_list:
    #     print(user['username'])

    print("\n ---------- \n")
    print('{} user(s) following you:'.format(len(followers_list)))
    print("\n ---------- \n")
    print_n_items_in_a_row(list(map(lambda u: u['username'], followers_list)), 10)
    # for user in followers_list:
    #     print(user['username'])

    # TODO: Check diff with latest and cached - show new unfollowing, followings
    # TODO: Store statistics data, draw charts

    # Broken - shows users who is actually following me, probably user map is diff
